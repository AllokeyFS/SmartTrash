...
class WasteRepositoryTest {
    ...
    @Test
    fun `searchWasteItems finds Plastic with typo plastc`() = runTest {
        every { wasteItemDao.getAllWasteItems() } returns flowOf(testItems)

        val results = mutableListOf<List<WasteItemEntity>>()
        repository.searchWasteItems("plastc").collect { results.add(it) }

        assertTrue(
            "Should find Plastic Bottle with typo plastc",
            results.any { list -> list.any { it.category == "Plastic" } }
        )
    }
    ...
    @Test
    fun `searchWasteItems returns empty for unrelated query`() = runTest {
        every { wasteItemDao.getAllWasteItems() } returns flowOf(testItems)

        val results = mutableListOf<List<WasteItemEntity>>()
        repository.searchWasteItems("xyzqwerty").collect { results.add(it) }
